            ...
            for layer_id in range(len(models[0].language_model.encoder.layers)):
                ...
                if use_exit_block:
                    if md.swiglu:
                        ...
                        message["exit block mlp l0 weight V"] = torch.cat([w[1] for w in exit_block_mlp_l0_weight], dim=0)
                    else:
                        message["exit block mlp l0 weight"] = torch.cat(exit_block_mlp_l0_weight, dim=0)
                    message["exit block qkv weight"] = torch.cat(exit_block_qkv_weight, dim=0)
                    message["exit block dense weight"] = torch.cat(exit_block_dense_weight, dim=1)
                    message["exit block mlp l1 weight"] = torch.cat(exit_block_mlp_l1_weight, dim=1)
                    if md.linear_bias:
                        message["exit block qkv bias"] = torch.cat(exit_block_qkv_bias, dim=0)
                        if md.swiglu:
                            for tp_rank in range(tp_size):
                                mlp_l0_bias[tp_rank] = torch.chunk(exit_block_mlp_l0_bias[tp_rank], 2, dim=0)
                            message["exit block mlp l0 bias W"] = torch.cat([b[0] for b in exit_block_mlp_l0_bias],dim=0)
                            message["exit block mlp l0 bias V"] = torch.cat([b[1] for b in exit_block_mlp_l0_bias],dim=0)
                        else:
                            message["exit block mlp l0 bias"] = torch.cat(exit_block_mlp_l0_bias, dim=0)
