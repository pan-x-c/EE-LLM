        ...
        for layer in range(len(models[0].language_model.encoder.layers)):
            ...
            if use_exit_block:
                ...
                if md.linear_bias:
                    ...
                    if md.swiglu:
                        exit_block_mlp_l0_bias_W = torch.chunk(msg.pop("exit block mlp l0 bias W"), args.target_tensor_parallel_size, dim=0)
                        exit_block_mlp_l0_bias_V = torch.chunk(msg.pop("exit block mlp l0 bias V"), args.target_tensor_parallel_size, dim=0)
                        exit_block_mlp_l0_bias = [torch.cat(bias, dim=0) for bias in zip(exit_block_mlp_l0_bias_W, exit_block_mlp_l0_bias_V)]
                    else:
                        exit_block_mlp_l0_bias = torch.chunk(msg.pop("exit block mlp l0 bias"), args.target_tensor_parallel_size, dim=0)
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                ...
                if use_exit_norm:
                    l.exit_norm.weight.data.copy_(exit_norm_weight)
                    if md.norm_has_bias:
                        l.exit_norm.bias.data.copy_(exit_norm_bias)
                if use_exit_mlp:
                    l.mlp.trunk.dense_h_to_4h.weight.data.copy_(mlp_l0_weight[tp_rank])
                    l.mlp.trunk.dense_4h_to_h.weight.data.copy_(mlp_l1_weight[tp_rank])
                    l.mlp.branch.dense_h_to_4h.weight.data.copy_(mlp_l0_exit_weight[tp_rank])
                    l.mlp.branch.dense_4h_to_h.weight.data.copy_(mlp_l1_exit_weight[tp_rank])
                else:
                    l.mlp.dense_h_to_4h.weight.data.copy_(mlp_l0_weight[tp_rank])
                    l.mlp.dense_4h_to_h.weight.data.copy_(mlp_l1_weight[tp_rank])
                if is_early_exit_layer and md.untie_exit_output_weights:
                    models[tp_rank].language_model.encoder.exit_output_weights[layer_num].data.copy_(exit_output_weight[tp_rank])
                if md.linear_bias:
                    l.self_attention.query_key_value.bias.data.copy_(qkv_bias[tp_rank])
                    l.self_attention.dense.bias.data.copy_(dense_bias)
                    if use_exit_mlp:
                        l.mlp.trunk.dense_h_to_4h.bias.data.copy_(mlp_l0_bias[tp_rank])
                        l.mlp.trunk.dense_4h_to_h.bias.data.copy_(mlp_l1_bias)
                        l.mlp.branch.dense_h_to_4h.bias.data.copy_(mlp_l0_exit_bias[tp_rank])
                        l.mlp.branch.dense_4h_to_h.bias.data.copy_(mlp_l1_exit_bias)
                    else:
                        l.mlp.dense_h_to_4h.bias.data.copy_(mlp_l0_bias[tp_rank])
                        l.mlp.dense_4h_to_h.bias.data.copy_(mlp_l1_bias)
                if use_exit_block:
                    l.exit_block.input_norm.weight.data.copy_(exit_block_input_norm_weight)
                    l.exit_block.post_attention_norm.weight.data.copy_(exit_block_post_norm_weight)
                    l.exit_block.self_attention.query_key_value.weight.data.copy_(exit_block_qkv_weight[tp_rank])
                    l.exit_block.self_attention.dense.weight.data.copy_(exit_block_dense_weight[tp_rank])
                    l.exit_block.mlp.dense_h_to_4h.weight.data.copy_(exit_block_mlp_l0_weight[tp_rank])
                    l.exit_block.mlp.dense_4h_to_h.weight.data.copy_(exit_block_mlp_l1_weight[tp_rank])
                    if md.norm_has_bias:
                        l.exit_block.input_norm.bias.data.copy_(exit_block_input_norm_bias)
                        l.exit_block.post_attention_norm.bias.data.copy_(exit_block_post_norm_bias)
                    if md.linear_bias:
                        l.exit_block.self_attention.query_key_value.bias.data.copy_(exit_block_qkv_bias[tp_rank])
                        l.exit_block.self_attention.dense.bias.data.copy_(exit_block_dense_bias)
                        l.exit_block.mlp.dense_4h_to_h.bias.data.copy_(exit_block_mlp_l1_bias)
                        l.exit_block.mlp.dense_h_to_4h.bias.data.copy_(exit_block_mlp_l0_bias[tp_rank])

            total_layer_num = total_layer_num + 1
            check_message(msg)

        if not md.output_layer and is_early_exit_stage and not (pre_process or post_process):
            for tp_rank in range(args.target_tensor_parallel_size):
